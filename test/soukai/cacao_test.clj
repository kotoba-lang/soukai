...
(deftest minted-cacao-signature-verifies
  (testing "the Ed25519 signature is over the exact SIWE message"
    (let [id    (c/generate-identity)
          grant {:cap :cap/read :scope "soukai"}
          opts  {:aud "https://kotobase.net" :nonce "n1" :issued-at "2026-07-12T00:00:00Z"}
...
(deftest minted-cacao-is-wellformed-cbor
  (let [id    (c/generate-identity)
        cacao (c/mint id {:cap :cap/transact :scope "soukai"}
                      {:aud "https://kotobase.net" :nonce "n2"
                       :issued-at "2026-07-12T00:00:00Z" :expiry "2026-07-12T01:00:00Z"})
...
(deftest per-actor-key-persists
  (testing "load-or-create! generates once, then reloads the same identity"
    (let [p  (str (System/getProperty "java.io.tmpdir") "/soukai-id-test-" (System/nanoTime) ".edn")
...
(deftest resources-encode-capability-and-graph
  (is (= ["kotoba://op/datom:read" "kotoba://graph/soukai"]
         (c/grant->resources {:cap :cap/read :scope "soukai"})))
